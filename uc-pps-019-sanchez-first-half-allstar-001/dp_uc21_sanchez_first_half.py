"""
============================================================================
GOVERNED DATA PRODUCT — USE CASE #22  (uc-pps-019)
"Cristopher Sanchez — First-Half 2026 All-Star Assessment (NL starter)"
============================================================================

Layer-3 BUILD artifact for the Phillies Pitching (pps) value stream.
Sibling of uc-pps-017 (Luzardo ASG retrospective, dp_uc17): results first,
then the indicators that plausibly drove them, then persona actions
(Sanchez / catcher / manager / pitching department).

Pattern lineage: UC3 -> UC8 -> UC11 -> UC18 (retrospective shape) ->
UC19/uc-pps-017 (pitcher ASG kernel) -> THIS (UC #22 / uc-pps-019).

Governance lineage (00/01/02/03 in this package folder):
  - data-product-owner     : claimed UC #22 / dp_uc21 / uc-pps-019
  - use-case-validator     : GO, no blocking gaps (01)
  - source-system-profiler : entity lock pitcher==650911 (resolved from data,
                             not hand-keyed); phils_2025+phils_2026; 2026
                             cache fresh to 07-12, last start 07-11
  - kpi-calculator         : locked cores inherited VERBATIM from dp_uc17
                             (get_stats/nresults, whiff, chase, putaway,
                             fpsr, hard_hit, CSW, outs/RA9/FIP, TTO,
                             battery, count leverage). NEW (spec'd in 02,
                             provisional): QR-1 quick-resolution rate,
                             QR-2 pitches/PA, QR-3 quick-resolution quality,
                             SL-1 scoreless-streak receipt
  - business-glossary-agent: no governed term redefined (03)

DATA WINDOW / FRESHNESS:
  * phils_2025 + phils_2026 parquet, phillies_role=='pitching',
    pitcher==650911, game_type=='R', dedup game_pk+at_bat_number+pitch_number.
  * 2026 window: 2026-03-26 .. 2026-07-11 (20 starts). Cache fresh 2026-07-12.
  * 2025 comparison season: full year, 32 starts.
  * MANUAL CARRY-INS: NL ASG starter designation; "50.2 IP scoreless streak
    = Phillies record" (record claim); 2025 NL CY runner-up.
  * IP from event outs (~±1 out vs official). Runs = on-mound score deltas
    (RA9, NOT official ERA). SL-1 streak is a consistency receipt only.

OUTPUTS (NEW files, none overwritten) -> <MLB repo>/out/:
  dp_uc21_sanchez_season_line.csv        2025 vs 2026 results (+IP, RA9, FIP)
  dp_uc21_sanchez_start_log_2026.csv     per-start log (+QR-1 per start)
  dp_uc21_staff_benchmark_2026.csv       Phillies 2026 (>=500 pitches) + QR-1
  dp_uc21_sanchez_arsenal_yoy.csv        usage/velo/mvt/whiff/putaway/xwOBA
  dp_uc21_sanchez_process_kpis_yoy.csv   FPSR/chase/zone/putaway/HH/CSW/whiff
  dp_uc21_sanchez_by_stand_yoy.csv       by batter stand YoY
  dp_uc21_sanchez_tto_yoy.csv            times-through-order YoY
  dp_uc21_sanchez_battery_2026.csv       catcher splits 2026
  dp_uc21_sanchez_monthly_2026.csv       monthly trend 2026
  dp_uc21_sanchez_count_leverage_yoy.csv 2-strike funnel + ahead-in-count
  dp_uc21_sanchez_qr_yoy.csv             QR-1/QR-2 by season (+monthly 2026)
  dp_uc21_sanchez_qr_quality_yoy.csv     QR-3: results by <=3 vs >=4 pitches
  dp_uc21_staff_qr_2026.csv              QR-1 staff benchmark
  dp_uc21_sanchez_streak_receipt.csv     SL-1 scoreless-run receipt
  dp_uc21_dq_scorecard.csv               data-quality-engineer scorecard
  dp_uc21_freshness_manifest.csv         source/window/fitness receipts
  dp_uc21_fig1_arsenal_yoy.png           fig 1 — YoY usage x whiff
  dp_uc21_fig2_start_trend.png           fig 2 — per-start xwOBA + runs (streak shaded)
  dp_uc21_fig3_quick_resolution.png      fig 3 — QR-1 monthly vs 2025 + staff band
============================================================================
"""
...
SANCHEZ = 650911                   # Cristopher Sanchez, MLBAM id (entity lock, data-derived)
CURRENT_YEAR = 2026
CARRY_INS = ("NL ASG starter designation; 50.2 IP scoreless streak = Phillies "
             "record (record claim); 2025 NL CY runner-up — user-provided")
...
PITCH_COLORS = {"Sinker": "#8C564B", "Changeup": "#FF7F0E", "Slider": "#1F77B4",
                "4-Seam Fastball": "#E81828", "Curveball": "#2CA02C", "Cutter": "#9467BD"}

# ===========================================================================
# LOADERS (inherited from dp_uc17)
# ===========================================================================
def _coerce(df):
...
# ===========================================================================
# LOCKED KPI FUNCTIONS — inherited verbatim from dp_uc17 (via dp_uc11).
# ===========================================================================
...
# ===========================================================================
# NEW KPIs — QR-1..QR-3 + SL-1 (specs in 02_engineering_design.md, provisional)
# ===========================================================================
def qr_frame(df):
    """PA-grain frame with resolved-pitch count. Population: completed PAs
    (locked definition: events not NA / pickoff_1b)."""
    last = pa_last(df)
    last = last[~last.events.replace(np.nan,"NA").isin(["NA","pickoff_1b"])].copy()
    last["quick"] = last.pitch_number <= 3
    return last

def qr_rate(level, df):
    """QR-1 + QR-2 at any grain. level applies to the PA-grain frame."""
    if isinstance(level, str): level=[level]
    last = qr_frame(df)
    g = last.groupby(level, as_index=False).agg(
        pa=("quick","size"), quick_pa=("quick","sum"))
    g["qr1_rate"] = (g.quick_pa/g.pa).round(3)
    # QR-2 pitches/PA needs pitch grain:
    pp = df.groupby(level, as_index=False).agg(pitches=("description","size")) \
        if all(l in df.columns for l in level) else None
    if pp is not None:
        g = g.merge(pp, on=level, how="left")
        g["pitches_per_pa"] = (g.pitches/g.pa).round(2)
    return g

def qr_quality(df):
    """QR-3: results by quick (<=3) vs long (>=4) buckets, per season."""
    last = qr_frame(df)
    key = last[["game_pk","at_bat_number","quick"]]
    d = df.merge(key, on=["game_pk","at_bat_number"], how="inner")
    q = nresults(["game_year","quick"], d)
    xw = d.groupby(["game_year","quick"],as_index=False).agg(
        xwoba=("estimated_woba_using_speedangle","mean")).round(3)
    q = q.drop(columns=[c for c in ["xwoba"] if c in q]).merge(xw,on=["game_year","quick"])
    ibb = last.groupby(["game_year","quick"],as_index=False).agg(
        intent_walks=("events", lambda s: int((s=="intent_walk").sum())))
    return q.merge(ibb, on=["game_year","quick"], how="left")

def scoreless_streak(df):
    """SL-1: longest run of consecutive event-map outs with zero on-mound
    runs, chronological. Receipt-class consistency check (see 02 caveats)."""
    last = pa_last(df).sort_values(["game_date","game_pk","at_bat_number"])
    best = cur = 0
    start = end = cur_start = None
    for _, r in last.iterrows():
        runs = max((r.post_bat_score or 0) - (r.bat_score or 0), 0)
        if runs > 0:
            if cur > best:
                best, start, end = cur, cur_start, prev_date
            cur, cur_start = 0, None
        else:
            o = OUTS_MAP.get(r.events, 0)
            if o and cur_start is None:
                cur_start = str(r.game_date)[:10]
            cur += o
        prev_date = str(r.game_date)[:10]
    if cur > best:
        best, start, end = cur, cur_start, prev_date
...
def main():
    sz = load_sanchez()
    d26 = sz[sz.game_year==2026].copy()
    d25 = sz[sz.game_year==2025].copy()
    w = pd.read_csv(WOBA_CSV).set_index("Season")

    # --- 1. season results line -------------------------------------------
    line = nresults(["game_year"], sz)
    extra = []
    ...
    xwy = sz.groupby("game_year",as_index=False).agg(
        xwoba=("estimated_woba_using_speedangle","mean")).round(3)
    line = line.drop(columns=[c for c in ["xwoba"] if c in line]).merge(xwy,on="game_year")
    hh = hard_hit_rate(["game_year"], sz)[["game_year","hard_hit_rate"]]
    line = line.merge(hh, on="game_year")
    line.to_csv(os.path.join(OUT_DIR,"dp_uc21_sanchez_season_line.csv"),index=False)

    # --- 2. per-start log 2026 (+QR-1 per start) ----------------------------
    rows=[]
    for gpk, g in d26.groupby("game_pk"):
        ...
        rows.append(dict(
            ...
            qr1=round(float(qf.quick.mean()),3)))
    slog = pd.DataFrame(rows).sort_values("game_date").reset_index(drop=True)
    slog.to_csv(os.path.join(OUT_DIR,"dp_uc21_sanchez_start_log_2026.csv"),index=False)

    # --- 3. staff benchmark 2026 (>=500 pitches) + QR-1 ---------------------
    ...
    qr_b = qr_rate(["player_name"], staff)[["player_name","pa","qr1_rate","pitches_per_pa"]]
    bench = bench.merge(qr_b.rename(columns={"pa":"pa_qr"}), on="player_name")
    bench.to_csv(os.path.join(OUT_DIR,"dp_uc21_staff_benchmark_2026.csv"),index=False)
    qr_b.sort_values("qr1_rate",ascending=False).to_csv(
        os.path.join(OUT_DIR,"dp_uc21_staff_qr_2026.csv"),index=False)
    ...
    ars.to_csv(os.path.join(OUT_DIR,"dp_uc21_sanchez_arsenal_yoy.csv"),index=False)
    ...
    proc.to_csv(os.path.join(OUT_DIR,"dp_uc21_sanchez_process_kpis_yoy.csv"),index=False)

    # --- 6. by stand YoY -----------------------------------------------------
    stand = nresults(["game_year","stand"], sz)
    xws = sz.groupby(["game_year","stand"],as_index=False).agg(
        xwoba=("estimated_woba_using_speedangle","mean")).round(3)
    stand = stand.merge(xws, on=["game_year","stand"])
    stand.to_csv(os.path.join(OUT_DIR,"dp_uc21_sanchez_by_stand_yoy.csv"),index=False)

    # --- 7. TTO YoY ----------------------------------------------------------
    sz_t = sz[sz.n_thruorder_pitcher.notna()].copy()
    sz_t["tto"] = sz_t.n_thruorder_pitcher.clip(upper=3).astype(int)
    tto = nresults(["game_year","tto"], sz_t)
    tto.to_csv(os.path.join(OUT_DIR,"dp_uc21_sanchez_tto_yoy.csv"),index=False)

    # --- 8. battery splits 2026 ---------------------------------------------
    ...
    bat = bat.merge(qr_rate(["catcher"],d26c)[["catcher","qr1_rate","pitches_per_pa"]],on="catcher")
    xwc = d26c.groupby("catcher",as_index=False).agg(
        xwoba=("estimated_woba_using_speedangle","mean")).round(3)
    bat = bat.drop(columns=[c for c in ["xwoba"] if c in bat]).merge(xwc,on="catcher")
    bat.to_csv(os.path.join(OUT_DIR,"dp_uc21_sanchez_battery_2026.csv"),index=False)
    ...
    mon = mon.merge(qr_rate(["month"],d26m)[["month","qr1_rate","pitches_per_pa"]],on="month")
    mon.to_csv(os.path.join(OUT_DIR,"dp_uc21_sanchez_monthly_2026.csv"),index=False)
    ...
    lev.to_csv(os.path.join(OUT_DIR,"dp_uc21_sanchez_count_leverage_yoy.csv"),index=False)

    # --- 11. QR-1/QR-2 YoY + monthly (THE use-case question) ------------------
    qy = qr_rate(["game_year"], sz)
    qy.insert(0,"grain","season")
    qm = qr_rate(["month"], d26m).rename(columns={"month":"game_year"})
    qm.insert(0,"grain","month_2026")
    qr_all = pd.concat([qy,qm],ignore_index=True)
    qr_all.to_csv(os.path.join(OUT_DIR,"dp_uc21_sanchez_qr_yoy.csv"),index=False)

    # --- 12. QR-3 quality split ------------------------------------------------
    qq = qr_quality(sz)
    qq.to_csv(os.path.join(OUT_DIR,"dp_uc21_sanchez_qr_quality_yoy.csv"),index=False)

    # --- 13. SL-1 streak receipt -----------------------------------------------
    best, s_start, s_end = scoreless_streak(d26)
    streak = pd.DataFrame([dict(metric="SL-1 longest scoreless run (event-outs, on-mound runs)",
        outs=best, ip=ip_str(best), window_start=s_start, window_end=s_end,
        carry_in="official record cited as 50.2 IP (user-provided)",
        note="score-delta method; consistency receipt, not record adjudication")])
    streak.to_csv(os.path.join(OUT_DIR,"dp_uc21_sanchez_streak_receipt.csv"),index=False)

    # --- 14. DQ scorecard + freshness manifest ----------------------------------
    key_cdes=["pitch_name","release_speed","plate_x","plate_z","zone","description",
              "events","stand","woba_value","woba_denom","fielder_2","pitch_number",
              "n_thruorder_pitcher","bat_score","post_bat_score"]
    dq=[dict(check="entity_lock",detail="pitcher==650911 only",
             result="PASS" if set(sz.pitcher.unique())=={SANCHEZ} else "FAIL"),
        dict(check="dedup",detail="game_pk+at_bat_number+pitch_number unique",
             result="PASS" if not sz.duplicated(["game_pk","at_bat_number","pitch_number"]).any() else "FAIL"),
        dict(check="game_type",detail="regular season only",
             result="PASS" if set(sz.game_type.unique())=={"R"} else "FAIL"),
        dict(check="weights_join",detail="wOBA weights joined for both seasons",
             result="PASS" if sz.wBB.notna().all() else "FAIL"),
        dict(check="qr_population",detail="pitch_number non-null on PA-final rows",
             result="PASS" if qr_frame(sz).pitch_number.notna().all() else "FAIL")]
    for c in key_cdes:
        nn = sz[c].notna().mean() if c in sz.columns else 0.0
        dq.append(dict(check=f"completeness:{c}",detail="pitch log non-null share",
                       result=round(float(nn),3)))
    pd.DataFrame(dq).to_csv(os.path.join(OUT_DIR,"dp_uc21_dq_scorecard.csv"),index=False)

    fresh=pd.DataFrame([
        dict(source="data/phillies/phils_2026.parquet",
             window=f"2026-03-26..{str(d26.game_date.max())[:10]} (cache to 2026-07-12)",
             rows=len(d26), note="2026 MLB pitch log, entity-locked, R games, 20 starts"),
        dict(source="data/phillies/phils_2025.parquet",
             window="2025-03-31..2025-09-28", rows=len(d25),
             note="2025 full-season comparison, 32 starts"),
        dict(source="user-provided context", window="2026-07-14", rows=0, note=CARRY_INS),
        dict(source="derived", window="n/a", rows=0,
             note="IP from event-outs; runs=on-mound score deltas (RA9 not ERA); SL-1 receipt-class"),
    ])
    fresh.to_csv(os.path.join(OUT_DIR,"dp_uc21_freshness_manifest.csv"),index=False)
    ...
    order = [p for p in ["Sinker","Changeup","Slider","4-Seam Fastball","Cutter"]
             if p in set(ars.pitch_name)]
    abbr = {"Sinker":"SI","Changeup":"CH","Slider":"SL","4-Seam Fastball":"FF","Cutter":"FC"}
    ...
    for ax,yr in zip(axes,(2025,2026)):
        ...
        ax.set_xticks(x); ax.set_xticklabels([abbr[o] for o in order],fontsize=9)
    ...
    fig.suptitle("Sanchez arsenal — sinker/changeup core, the slider's growing role",
                 color=PHI_NAVY,weight="bold")
    fig.tight_layout()
    fig.savefig(os.path.join(OUT_DIR,"dp_uc21_fig1_arsenal_yoy.png"),dpi=160)
    plt.close(fig)

    # fig 2: per-start xwOBA + runs, streak shaded
    fig,ax=plt.subplots(figsize=(11,4.6))
    x=np.arange(len(slog))
    if s_start and s_end:
        in_win = [(s_start <= dt <= s_end) for dt in slog.game_date]
        idx = [i for i,b in enumerate(in_win) if b]
        if idx:
            ax.axvspan(min(idx)-.5, max(idx)+.5, color=PHI_LGRAY, alpha=.45, zorder=1,
                       label=f"scoreless run window (SL-1: {ip_str(best)} IP)")
    ax.bar(x,slog.runs,color=PHI_GRAY,zorder=2,label="runs while on mound")
    ax2=ax.twinx()
    ...
    ax.set_title("Sanchez 2026, start by start — contact quality allowed vs runs",
                 color=PHI_NAVY,weight="bold")
    fig.tight_layout()
    fig.savefig(os.path.join(OUT_DIR,"dp_uc21_fig2_start_trend.png"),dpi=160)
    plt.close(fig)

    # fig 3: QR-1 monthly vs 2025 baseline + staff band
    fig,ax=plt.subplots(figsize=(9,4.6))
    months = qm.game_year.tolist(); vals = qm.qr1_rate.tolist(); pas = qm.pa.tolist()
    q25 = float(qy[qy.game_year==2025].qr1_rate.iat[0])
    q26 = float(qy[qy.game_year==2026].qr1_rate.iat[0])
    staff_med = float(qr_b.qr1_rate.median())
    ax.bar(np.arange(len(months)),vals,color=PHI_RED,zorder=2,width=.55)
    for i,(v,p_) in enumerate(zip(vals,pas)):
        ax.text(i,v+.008,f"{v:.1%}\n({int(p_)} PA)",ha="center",fontsize=8)
    ax.axhline(q25,color=PHI_GRAY,ls="--",lw=1.4,label=f"2025 full season {q25:.1%}")
    ax.axhline(q26,color=PHI_NAVY,ls="-",lw=1.4,label=f"2026 first half {q26:.1%}")
    ax.axhline(staff_med,color=PHI_NAVY,ls=":",lw=1.2,alpha=.6,
               label=f"2026 staff median (≥500 p) {staff_med:.1%}")
    ax.set_xticks(np.arange(len(months))); ax.set_xticklabels(months)
    ax.set_ylabel("share of PA resolved in ≤3 pitches")
    ax.set_ylim(0,.68); ax.grid(axis="y",color=PHI_LGRAY,zorder=0)
    ax.legend(fontsize=8,loc="lower right")
    ax.set_title("QR-1: quick-resolution rate — the pitching department's tempo metric",
                 color=PHI_NAVY,weight="bold")
    fig.tight_layout()
    fig.savefig(os.path.join(OUT_DIR,"dp_uc21_fig3_quick_resolution.png"),dpi=160)
    plt.close(fig)

    # copy figures into the package folder for the report
    for f in ["dp_uc21_fig1_arsenal_yoy.png","dp_uc21_fig2_start_trend.png",
              "dp_uc21_fig3_quick_resolution.png"]:
        try: shutil.copy(os.path.join(OUT_DIR,f), os.path.join(HERE,"out_"+f))
        except Exception as e: print("fig copy skipped:", f, e)

    # --- console receipts ---------------------------------------------------------
    pd.set_option("display.width", 240)
    print("UC-PPS-019 build complete.\n")
    print("SEASON LINE\n", line.to_string(index=False), "\n")
    print("START LOG 2026\n", slog.to_string(index=False), "\n")
    print("STAFF BENCHMARK\n", bench[["player_name","pitches","woba","xwoba","csw_rate","qr1_rate","pitches_per_pa"]].to_string(index=False), "\n")
    ...
    print("BATTERY\n", bat[["catcher","pitches","plate_apps","woba","xwoba","csw_rate","qr1_rate"]].to_string(index=False), "\n")
    print("MONTHLY\n", mon[["month","plate_apps","woba","xwoba","krate","csw_rate","qr1_rate","pitches_per_pa"]].to_string(index=False), "\n")
    print("COUNT LEVERAGE\n", lev.to_string(index=False), "\n")
    print("QR YoY + MONTHLY\n", qr_all.to_string(index=False), "\n")
    print("QR QUALITY\n", qq.to_string(index=False), "\n")
    print("STREAK RECEIPT\n", streak.to_string(index=False), "\n")

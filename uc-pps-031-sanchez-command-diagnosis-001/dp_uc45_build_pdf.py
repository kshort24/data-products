"""Markdown -> HTML -> weasyprint, Phillies-branded. House-standard PDF path (inherited from dp_uc44_build_pdf.py)."""
import base64, re
...
SRC  = HERE / 'dp_uc45_sanchez_command_report.md'
DEST = HERE / 'dp_uc45_sanchez_command_report.pdf'
...
@page {{ size: Letter; margin: 14mm 13mm 15mm 13mm;
  @bottom-center {{ content: "uc-pps-031 · dp_uc45 · v1.0.0 · Cristopher Sánchez command diagnosis · data through 2026-09-15 · page " counter(page) " of " counter(pages);
                    font-family: Helvetica, Arial, sans-serif; font-size: 7pt; color: #888; }} }}
body {{ font-family: Helvetica, Arial, sans-serif; font-size: 8.7pt; line-height: 1.45; color: #1b1b1b; }}
h1 {{ color: {NAVY}; font-size: 15.5pt; margin: 0 0 1mm 0; }}
h3 {{ color: {BLUE}; font-size: 10.5pt; margin: 0 0 3mm 0; border-bottom: 3px solid {RED}; padding-bottom: 2mm;
...
h2 + p, h2 + table, h2 + blockquote {{ page-break-before: avoid; }}
p {{ margin: 0 0 2.4mm 0; }}
strong {{ color: #000; }}
blockquote {{ background: #FFF4F5; border-left: 4px solid {RED}; margin: 3mm 0; padding: 2.6mm 3.4mm;
  font-size: 8.2pt; color: #3a2326; page-break-inside: avoid; }}
blockquote p {{ margin: 0 0 1.6mm 0; }} blockquote p:last-child {{ margin-bottom: 0; }}
blockquote ul {{ margin: 0; }}
table {{ border-collapse: collapse; width: 100%; margin: 2.6mm 0 3.6mm 0; font-size: 7.4pt;
...
pre {{ background: #f1f3f6; padding: 2.4mm 3mm; border-left: 3px solid {BLUE}; margin: 2.5mm 0;
       page-break-inside: avoid; }}

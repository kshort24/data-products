"""
dp_uc40 — PDF renderer for the Trea Turner 2026 recency read.
...
branded table headers, blockquote warning box. Follows dp_uc37_build_pdf.py. Figures live in ./out, so base_url points there.

Run from this directory so base_url resolves the dp_uc40_fig*.png figures.
"""
...
MD = os.path.join(HERE, "dp_uc40_turner_recency_report.md")
FIGDIR = os.path.join(HERE, "out")
PDF = os.path.join(HERE, "dp_uc40_turner_recency_report.pdf")
...
@page {{
    ...
    @bottom-center {{
        content: "Trea Turner — The Power Outage · as of 2026-09-02 · UC #40 / uc-pos-014 / dp_uc40 · page " counter(page) " of " counter(pages);
...
/* the first rule closes the title/data-window page deliberately */
hr:first-of-type {{ page-break-after: always; border-top: none; margin: 0; }}
...
def main():
    ...
    HTML(string=html, base_url=FIGDIR + os.sep).write_pdf(PDF)
